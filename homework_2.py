...
db.pretty_print("SELECT COUNT(*) as ProtectedAACCount FROM Track INNER JOIN MediaType ON Track.MediaTypeId=MediaType.MediaTypeId Where MediaType.Name = 'Protected AAC audio file'")
...
db.pretty_print("SELECT SUM(Invoice.Total) as InvoiceSum, Customer.FirstName, Customer.LastName FROM Invoice INNER JOIN Customer ON Invoice.CustomerId=Customer.CustomerId GROUP BY Invoice.CustomerId ORDER BY InvoiceSum DESC LIMIT 1")
